...
const closeEditModal = () => store.commit('closeEditModal')
...
watch(
  () => props.word,
  (newWord) => {
    wordData.en = newWord.en || ''
    wordData.ua = newWord.ua || ''
    wordData.category = newWord.category || 'noun'
    wordData.isIrregular = newWord.isIrregular || false
    selectCategory.value = newWord.category || 'noun'
    selectVerbType.value = newWord.isIrregular ? 'irregular' : 'regular'
  },
  { immediate: true }
)
